...
public class AuctionManagerImpl implements AuctionManager {
	 ...
	 * @return 物品对应的全部竞价
	 */
	public List<BidBean> getBidByItem(Integer itemId) throws AuctionException {
		try {
			List<Bid> l = bidDao.findByItem(itemId);
			List<BidBean> result = new ArrayList<BidBean>();
			for (int i = 0; i < l.size(); i++) {
				Bid bid = l.get(i);
				BidBean bb = new BidBean();
			...
			log.debug(e.getMessage());
			throw new AuctionException("浏览物品的全部竞价出现异常,请重试");
		}
	}
	 ...
	 *            物品id
	 */
	public void delItem(Integer id) throws AuctionException {
		try {
			itemDao.delete(id);
		} catch (Exception e) {
			log.debug(e.getMessage());
	 ...
	 *            种类id
	 */
	public void delKind(Integer id) throws AuctionException {
		try {
			kindDao.delete(id);
		} catch (Exception e) {
			log.debug(e.getMessage());
			throw new AuctionException("删除种类出现异常,请重试");
		}
	}
	...
	public List<ItemBean> getAllItemsByKind(int kindId) throws AuctionException {
		List<ItemBean> result = new ArrayList<ItemBean>();
		try {
			List<Item> items = itemDao.findAllItemByKind(kindId);
			for (Item item : items) {
				ItemBean ib = new ItemBean();
				initItem(ib, item);
				result.add(ib);
	 ...
	 *            竞价id;
	 */
	public void delBid(int bidId) throws AuctionException {
		try {
			bidDao.delete(bidId);
		} catch (Exception e) {
			log.debug(e.getMessage());
			throw new AuctionException("删除竞价出现异常,请重试");
		}
	}
