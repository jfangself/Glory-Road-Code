...
public class ViewBid extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		bidList.setOnItemLongClickListener(new OnItemLongClickListener() {
			@Override
			public boolean onItemLongClick(AdapterView<?> parent, View view,
					int position, long id) {
				// 获取被单击的列表项
				Long bidId = bidList.getAdapter().getItemId(position);
				String url = HttpUtil.BASE_URL + "delBid.jsp?bidId=" + bidId;
				DialogUtil.showDialog(ViewBid.this, "确认删除?", url);
				return true;
			}
		});
	}
	...
	private void viewBidDetail(int position) {
		...
		EditText bidPrice = (EditText) detailView.findViewById(R.id.bidPrice);
		EditText bidTime = (EditText) detailView.findViewById(R.id.bidTime);
		EditText bidUser = (EditText) detailView.findViewById(R.id.bidUser);
