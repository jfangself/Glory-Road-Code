...
public class ManageKind extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		kindList.setOnItemLongClickListener(new OnItemLongClickListener() {
			@Override
			public boolean onItemLongClick(AdapterView<?> parent, View view,
					int position, long id) {
				// 获取被单击的列表项
				Long kindId = kindList.getAdapter().getItemId(position);
				String url = HttpUtil.BASE_URL + "delKind.jsp?kindId=" + kindId;
				DialogUtil.showDialog(ManageKind.this, "确认删除?", url);
				return true;
			}
		});
	}
