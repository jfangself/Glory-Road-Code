...
public class AddBid extends Activity {
	// 定义界面中文本框
	EditText itemName, itemDesc, itemRemark, itemKind, initPrice, maxPrice,
			endTime, bidPrice;
	// 定义界面中两个按钮
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		bnAdd.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				try {
					// 执行类型转换
					double curPrice = Double.parseDouble(bidPrice.getText()
							.toString());
					// 执行输入校验
					if (curPrice < jsonObj.getDouble("maxPrice")) {
						DialogUtil.showDialog(AddBid.this, "您输入的竞价必须高于当前竞价",
								false);
					} else {
						// 添加竞价
						String result = addBid(jsonObj.getString("id"),
								curPrice + "");
						// 显示对话框
						DialogUtil.showDialog(AddBid.this, result, true);
					}
