...
function ProcessReq() {
	SetTabReq();
	window.IsCollapsible = $id("CollapsibleViewReq").checked;
	var json = $id("req").value;
	var html = "";
	try {
		if (json == "")
			json = "\"\"";
		...
		$id("CanvasReq").innerHTML = "<PRE class='CodeContainer'>" + html
				+ "</PRE>";
	} catch (e) {
		alert("JSON数据格式不正确:\n" + e.message);
		$id("CanvasReq").innerHTML = "";
	}
}
...
function CollapsibleViewClickedReq() {

	$id("CollapsibleViewDetailReq").style.visibility = $id("CollapsibleViewReq").checked ? "visible"
			: "hidden";
...
function QuoteKeysClickedReq() {

	window.QuoteKeys = $id("QuoteKeysReq").checked;
...
function CollapseAllClickedReq() {

	EnsureIsPopulatedReq();

	TraverseChildren($id("CanvasReq"), function(element) {
...
function CollapseLevelReq(level) {

	EnsureIsPopulatedReq();

	TraverseChildren($id("CanvasReq"), function(element, depth) {
...
function EnsureIsPopulatedReq() {

	if (!$id("CanvasReq").innerHTML && !!$id("Req").value)
		ProcessReq();
...
function SelectAllClickedReq() {
	...
	if (!!range.selectNode)
		...
		range.selectNode($id("CanvasReq"));

	else if (range.moveToElementText)

		range.moveToElementText($id("CanvasReq"));

	if (!!range.select)

		range.select($id("CanvasReq"));
